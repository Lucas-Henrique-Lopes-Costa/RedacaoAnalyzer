...
def analyze_with_trained_model(text):
    ...
    try:
        payload = {"text": text}
        response = requests.post(f"{TRAINED_MODEL_SERVICE_URL}/analyze-competencies", json=payload)
...
def generate_feedback_with_llama(text, analysis_data):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Erro na geração de feedback: {response.status_code} - {response.text}")
            return None
    except Exception as e:
